...
from networks.net_util import AGGGate
from networks.loss2 import FocalLoss
...
class GFSS_Model_AGG(nn.Module):
    '''
        Segmenter for Generalized Few-shot Semantic Segmentation
    '''
    def __init__(self, n_base, criterion=None, norm_layer=nn.BatchNorm2d, use_base=True, is_ft=False, n_novel=0, **kwargs):
        super(GFSS_Model_AGG, self).__init__()
        d_model = 512
        self.backbone = get_backbone(norm_layer=norm_layer, **kwargs)
        self.decoder = PSPModule(2048, out_features=d_model, norm_layer=norm_layer)
        self.classifier = nn.Sequential(
    ...
    def forward(self, img, mask=None, segtruth=None, img_b=None, mask_b=None):
        '''
            img       : [BxCxHxW]
            mask      : [BxHxW]
        '''
        if self.is_ft:
            ...
            return self.forward_base(img, mask, segtruth)
    ...
    def forward_base(self, img, mask=None, segtruth=None):
        '''
            img       : [BxCxHxW]
            mask      : [BxHxW]
        '''
        x4, x3, _  = self.backbone.base_forward(img, return_list=True)
        # x4 size is torch.Size([8, 2048, 60, 60])
        # x3 size is torch.Size([8, 1024, 60, 60])
        features = x4
        features = self.decoder(features) # [BxCxhxw]
        # features = self.backbone.base_forward(img)
        B, C, h, w = features.shape
        ...
        #TODO add segtruth
        #features size is torch.Size([8, 2048, 60, 60])
        features_tmp = x3
        x_decoder3 = self.decoder3(features_tmp)
        # x_decoder2 size is torch.Size([8, 64, 240, 240])
        x_decoder2 = self.decoder2(x_decoder3)
        x_pre_mid = F.interpolate(preds, size=(x_decoder2.size()[2], x_decoder2.size()[3]), mode='bilinear', align_corners=True)
        ...
        if self.criterion is not None and mask is not None:
            cls_emb = F.normalize(cls_emb, p=2, dim=-1).squeeze(0) # [KbasexC]
            proto_sim = torch.matmul(cls_emb, cls_emb.t()) # [KbasexKbase]
            return self.criterion(preds, mask, proto_sim=proto_sim, seg_pred=x_feat, segtruth=segtruth)
        else:
            return preds
...
class GFSS_Model_AGG_coco(nn.Module):
    '''
        Segmenter for Generalized Few-shot Semantic Segmentation
    '''
    def __init__(self, n_base, criterion=None, norm_layer=nn.BatchNorm2d, use_base=True, is_ft=False, n_novel=0, **kwargs):
        super(GFSS_Model_AGG_coco, self).__init__()
        d_model = 512
        self.backbone = get_backbone(norm_layer=norm_layer, **kwargs)
        self.decoder = PSPModule(2048, out_features=d_model, norm_layer=norm_layer)
        self.classifier = nn.Sequential(
        ...
        self.agggate = AGGGate(rgb_in_planes=64, disp_in_planes=61, bn_momentum=0.1)
        self.decoder3 = DecoderBlock(1024, 256)
        self.decoder2 = DecoderBlock(256, 64)
        self.aux_physical_regularization = nn.Sequential(
            nn.ConvTranspose2d(125, 32, 4, 2, 1),
            nonlinearity,
            nn.Conv2d(32, 16, 3, padding=1),
            nonlinearity,
            nn.Conv2d(16, 2, 3, padding=1),
...
class GFSS_Model_MultiTask(nn.Module):
    '''
        Segmenter for Generalized Few-shot Semantic Segmentation
    '''
    def __init__(self, n_base, criterion=None, norm_layer=nn.BatchNorm2d, use_base=True, is_ft=False, n_novel=0, **kwargs):
        super(GFSS_Model_MultiTask, self).__init__()
        d_model = 512
        self.backbone = get_backbone(norm_layer=norm_layer, **kwargs)
        self.decoder = PSPModule(2048, out_features=d_model, norm_layer=norm_layer)
        self.classifier = nn.Sequential(
    ...
    def forward_base(self, img, mask=None, segtruth=None):
        ...
        #x_feat size is torch.Size([16, 85, 240, 240])
        # x_feat = self.agggate(x_feat_agg)
        x_feat = self.aux_physical_regularization(x_decoder2)
        x_feat = F.interpolate(x_feat, size=(h, w), mode='bilinear', align_corners=True)
