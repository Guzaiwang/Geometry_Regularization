...
class AGGGate(nn.Module):
    def __init__(self, rgb_in_planes, disp_in_planes, bn_momentum=0.0003):
        ...
        self.rgb_to_feat = nn.Conv2d(self.rgb_in_planes, self.rgb_in_planes+self.disp_in_planes, kernel_size=1, bias=False)
        self.disp_to_feat = nn.Conv2d(self.disp_in_planes, self.rgb_in_planes+self.disp_in_planes, kernel_size=1, bias=False)

        self.feat_to_rgb = nn.Conv2d(self.rgb_in_planes+self.disp_in_planes, self.rgb_in_planes, kernel_size=1, bias=False)
        self.feat_to_disp = nn.Conv2d(self.rgb_in_planes+self.disp_in_planes, self.disp_in_planes, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        attention_vector = torch.cat([attention_vector_l, attention_vector_r], dim=1)
        # attention_vector = self.softmax(attention_vector)
        # print("attention_vector size", attention_vector.size())
        attention_vector_l, attention_vector_r = attention_vector[:, 0:1, :, :], attention_vector[:, 1:2, :, :]
        # print("attention_vector_l size", attention_vector_l.size())
        # print("attention_vector_r size", attention_vector_r.size())
        merge_feature = self.rgb_to_feat(rgb*attention_vector_l) + self.disp_to_feat(disp*attention_vector_r)

        rgb_sub_merge = self.feat_to_rgb(merge_feature)
        disp_sub_merge = self.feat_to_disp(merge_feature)

        rgb_out = (rgb + rgb_sub_merge) / 2
        disp_out = (disp + disp_sub_merge) / 2
