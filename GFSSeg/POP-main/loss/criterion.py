...
from loss.loss2 import FocalLoss
...
class OrthLoss(nn.Module):
    ...
    def forward(self, preds, target, is_ft=False, proto_sim=None, aux_preds=None, seg_pred=None, segtruth=None):
        '''
            pred      : [BxKaxhxw]
            target    : [BxHxW]
            proto_sim:   : [K1xK2]
        ...
        if segtruth is not None:
            focalloss = FocalLoss(gamma=2, alpha=0.25)
            focal_loss = focalloss(segtruth, seg_pred)
            total_loss = total_loss + focal_loss
            loss_dict['total_loss'] = total_loss
            loss_dict['sgetruth_loss'] = focal_loss
        return loss_dict
