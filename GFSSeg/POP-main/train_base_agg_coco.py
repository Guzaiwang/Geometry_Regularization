...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        trainset = eval('dataset.' + args.dataset + '.GFSSegTrain_agg')(
        ...
        if args.start_epoch > 0:
            seg_model = eval('networks.' + args.model + '.GFSS_Model_AGG_coco')(
        ...
        for epoch in range(args.start_epoch, args.num_epoch):
            ...
            for i, data in enumerate(train_loader):
                global_iteration += 1
                img, mask, cls_idx, segtruth = data
                img, mask, segtruth = img.cuda(non_blocking=True), mask.cuda(non_blocking=True), segtruth.cuda(non_blocking=True)
                ...
                with torch.cuda.amp.autocast(enabled=args.fp16):
                    loss_dict = model(img, mask, segtruth)
            ...
            if use_val and (epoch % 2 == 0 or epoch == args.num_epoch-1):
