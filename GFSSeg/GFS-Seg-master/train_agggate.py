...
from model.capl import PSPNet_multitask as PSPNet
...
def get_new_proto(val_supp_loader, model, base_num=16, novel_num=5, init_gen_proto=False):
    ...
    with torch.no_grad():
        ...
        for epoch in range(new_proto_num_epoch):
            for i, (input, target, _, _, aux_label) in enumerate(val_supp_loader):
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    optimizer = torch.optim.SGD(
        [{'params': model.layer0.parameters()},
         ...
         {'params': model.decoder2.parameters(), 'lr': args.base_lr * 10},
         {'params': model.decoder3.parameters(), 'lr': args.base_lr * 10},
         {'params': model.agggate.parameters(), 'lr': args.base_lr * 10},
         {'params': model.aux_physical_regularization.parameters(), 'lr': args.base_lr * 10},
         ],
    ...
    val_data = dataset.SemData_agg(split='val', data_root=args.data_root, data_list=args.val_list, \
          transform=val_transform, shot=args.shot, seed=args.manual_seed, \
          data_split=args.data_split, use_coco=args.use_coco, saved_path=args.saved_npy_path)
    val_sampler = None
    ...
    for val_supp_seed in val_supp_seed_list:
        ...
        val_supp_data = dataset.SemData_agg(split='val_supp', data_root=args.data_root, data_list=args.train_list, \
            transform=val_transform, shot=args.shot, seed=val_supp_seed, \
            data_split=args.data_split, use_coco=args.use_coco, val_shot=args.shot, saved_path=args.saved_npy_path)      
    ...
    train_data = dataset.SemData_agg(split='train', data_root=args.data_root, data_list=args.train_list, \
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and (epoch % 2 == 0) and (epoch >= args.start_val_epoch or (epoch % 4 == 0 and not args.use_coco)): 
            mean_all_mIoU = 0
            mean_base_mIoU = 0
            mean_novel_mIoU = 0
            for val_supp_loader in val_supp_loader_list:
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target, _, _, aux_label) in enumerate(train_loader):
        ...
        if args.zoom_factor != 8:
            ...
            aux_label = F.interpolate(target.unsqueeze(1).float(), size=(h, w), mode='bilinear', align_corners=True).squeeze(1).long()
        input = input.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        aux_label = aux_label.cuda(non_blocking=True)
        output, main_loss, aux_loss = model(x=input, y=target, aux_seg=aux_label, iter=i)
...
def validate(val_supp_loader, val_loader, model, criterion, novel_num, base_num, gened_proto):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, ori_size, ori_label,_) in enumerate(val_loader):
            data_time.update(time.time() - end)
            input = input.cuda(non_blocking=True)
            target = target.cuda(non_blocking=True)
            ...
            for b in range(input.size(0)):
                ...
                img_out = input[b]
                img_out  = F.interpolate(img_out.unsqueeze(0).detach(), size=tmp_target.size()[1:], mode='bilinear', align_corners=True)
                print("tmp_output size is", tmp_output.size(),tmp_output.max(), tmp_output.min() )
                print("tmp_target size is", tmp_target.size(), tmp_target.max(), tmp_target.min())
                print("input[b] size is", img_out.size())
                print("args.classes, args.ignore_label", args.classes, args.ignore_label)
